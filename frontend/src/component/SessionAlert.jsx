...
const calculateSessions = (schedule_array) => {
	...
	let sessions = schedule_array.filter(
		(session) => weekday[today.getDay()] === session.day,
	);
...
const checkHelpSessionURL = (schedule_array) => {
	const URLs = [];
	const sessions = schedule_array.filter(
		(session) => session.week === calculateCurrentWeek(),
	);
	if (sessions.length > 0) {
		for (let session of sessions[0].schedule_help_sessions()) {
			const curr_session = checkCurrentSession(session);
			if (
				curr_session.call_url_h &&
				weekday[today.getDay()] === curr_session.day
			) {
				URLs.push(curr_session.call_url_h);
			}
		}
									...
									>
										<CardContent>
											...
											<Typography variant="body2">
												A {card.title.toLowerCase()} is currently live! Click on
												the card to redirect yourself to the session.
											</Typography>
										</CardContent>
									</Card>
								);
