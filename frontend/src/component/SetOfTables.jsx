...
const SetOfTables = ({ boxes, lectures }) => {
	var durationCount = 0;
	const [noCatchup, setNoCatchup] = React.useState(
		localStorage.hasOwnProperty("eckles_noCatchup")
			? parseInt(localStorage.getItem("eckles_noCatchup"))
			: 1,
	);
	const [studentType, setStudentType] = React.useState(
		localStorage.hasOwnProperty("eckles_studentType")
			? localStorage.getItem("eckles_studentType")
			: "recommended",
	);
		...
		<>
			{lectures && (
				...
				>
					...
					{Object.keys(RELEVANCE).map((item, idx) => {
						if (RELEVANCE[item].select) {
							return (
								...
								>
									<input type="radio" checked={studentType === item} />
					...
					>
						<input type="checkbox" checked={noCatchup} />I completed COMP1531{" "}
						<span
							onClick={(e) => {
								alert(
									"If you uncheck this box, we will show all COMP1531 lectures that are relevant to this course. You won't necessarily need to watch every catch up lecture, because you may already have that knowledge.",
								);
								e.stopPropagation();
							}}
						>
			...
			{boxes.map(
				({ title, key, headers, table, maxWidth, totalDuration }, boxKey) => {
					var hasLectures = false;
					let totalWidth = 0;
					headers
						.filter((h) => h.showFn === undefined || h.showFn())
						.map((h) => h.width)
						.forEach((w) => {
							totalWidth += w;
						});
							...
							>
								...
								<AccordionDetails>
											...
											<TableBody>
												{table.map((tableRow, tableRowKey) => {
													...
													if (!window.location.href.includes("help-session")) {
														var curDuration = parseInt(
															tableRow[3].value.replace(/^\D+/g, ""),
														);
														if (window.location.href.includes("tutorial")) {
															curDuration = parseInt(
																tableRow[2].value.replace(/^\D+/g, ""),
															);
														}
														if (tableRowKey === 0) {
									...
									{hasLectures === true && (
										...
										>
											Total duration:{" "}
											<strong>{parseMins(durationCount)}</strong>
										</Typography>
									)}
								</AccordionDetails>
							</Accordion>
