...
const buildRelatedTutorials = (tutorial, allTutorials, term) => {
  ...
  const sameTopic = topicName
    ? allTutorials.filter(
        (t) => t.key !== tutorial.key && t.topic && t.topic().name === topicName
      )
    : [];
  const sameWeek = weekNumber
    ? allTutorials.filter(
        (t) => t.key !== tutorial.key && t.week && t.week().week === weekNumber
      )
    : [];
  ...
  return list.map((item) => {
    ...
    return {
      ...
      imageUrl: item.video_url
        ? `https://img.youtube.com/vi/${getYoutubeCodeFromUrl(
            item.video_url
          )}/hqdefault.jpg`
        : undefined,
...
const ContentTutorialsSingle = () => {
  ...
  React.useEffect(() => {
    if (tutorial !== null) {
      ...
      setRelatedTutorials(
        buildRelatedTutorials(
          tutorial,
          getters.content.content_tutorials,
          getters.term
        )
      );
    }
